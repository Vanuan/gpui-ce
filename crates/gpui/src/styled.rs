...
pub trait Styled: Sized {
    ...
    fn text_color(mut self, color: impl IntoHsla) -> Self {
        self.text_style().color = Some(color.into_hsla());
        self
    }
    ...
    ///
    /// This value cascades to its child elements.
    fn text_bg(mut self, bg: impl IntoHsla) -> Self {
        self.text_style().background_color = Some(bg.into_hsla());
        self
    }
    ...
    fn text_decoration_color(mut self, color: impl IntoHsla) -> Self {
        let style = self.text_style();
        let underline = style.underline.get_or_insert_with(Default::default);
        underline.color = Some(color.into_hsla());
        self
    }
